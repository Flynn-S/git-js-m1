...
console.log(exC);
/* Ex.D
    Create a variable me containing and object with the current information: 
...
function whoIsBigger(num1,num2) {
    return num1 > num2 ? console.log(`${num1} is bigger`) : console.log(num2 + " is bigger");
}
...
/* Ex.4
    ...
    If the boolean is true it should return the string without the 
    first letter, otherwise it should remove the last one.
*/
...
*/
const onlyLetters = stringWithNums => stringWithNums.replace(/[0-9]/g,'');
...
function isThisAnEmail(email) {
    // if (str.includes("@") && (str.includes('.co') || str.includes('.com'))) {
    //     return true
    // } else {
    //     return false
    // }
...
/* Ex.8
    ...
    It should use the Dice function defined in Ex1 and return an object that contains 
    both the sum of all values extracted and the single values of the dicerolls themselves.
...
function rollTheDices(num) {
    ...
    for (i=0;i < num;i++) {
        diceRolls.values.push(dice());
        diceRolls.sum += diceRolls.values[i];
    }
    return diceRolls
}
...
function howManyDays() {
    const d1 = new Date()
    const d2 = new Date()
    const numDays = d2.getTime() - d1.getTime()
    
    return Math.floor(numDays / (1000*60*60*24))
}

console.log(howManyDays("2021,02,05"));
/* Ex.10
...
function olderMovie() {
    ...
    for (let x =0;x < movies.length;x++) {
        if (movieYear[0] == movies[x].Year) {return movies[x].Title;}
    }  
}
...
function onlyThisMillennium () {
    ...
    for (let j=0;j<movies.length;j++) {
        if (movies[j].Year >= 2000) {
            moviesMillennium.push(movies[j].Title)
        }   
    } 
    console.log(moviesMillennium)
}
...
function searchMovie(string){
    ...
    for (let i =0;i<movies.length;i++) {
        if (movies[i].Title.toLowerCase().includes(string.toLowerCase())){
            searchMovies.push(movies[i].Title)
        }
    }
    return searchMovies
}
...
/* Ex.19
    ...
    with an array "match" with all the movies that contains the given string in the title, 
    and another array "nonMatch" with all the other movies.
*/
function searchAndDivide(string) {
    ...
    for (let i =0;i<movies.length;i++) {
        if (movies[i].Title.toLowerCase().includes(string.toLowerCase())){
            matchObj.match.push(movies[i].Title)
        } else {
            matchObj.nonMatch.push(movies[i].Title)
        }
    } return matchObj
}
